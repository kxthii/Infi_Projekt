...
class SportgruppeSportler(db.Model):
    ...
    Sportgruppe_Sportler_ID = db.Column(db.Integer, primary_key=True, unique=True)
    Sportler_ID = db.Column(db.ForeignKey('sportler.Sportler_ID'), index=True)
    Sportgruppen_ID = db.Column(db.ForeignKey('sportgruppe.Sportgruppen_ID'), index=True)

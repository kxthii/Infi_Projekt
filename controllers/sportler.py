...
def sportler_edit():
    ...
    sportler_to_edit = session.query(Sportler).filter(Sportler.Sportler_ID == Sportler_ID).first()
    
    if request.method == "POST":
        if edit_sportler_form.validate_on_submit():
            Sportler_ID = edit_sportler_form.Sportler_ID.data
            sportler_to_edit = db.session.query(Sportler).filter(Sportler.Sportler_ID == Sportler_ID).first()
...
def deleteSportler():
    ...
    if delete_item_form_obj.validate_on_submit():
        ...
        sportler_to_delete = db.session.query(Sportler).filter(Sportler.Sportler_ID == sportler_code_to_delete)

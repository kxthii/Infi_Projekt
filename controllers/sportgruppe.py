...
def sportgruppe():
    ...
    sportgruppe = session.query(Sportgruppe).order_by(Sportgruppe.Sportgruppen_ID).all()
...
def sportgruppe_edit():
    ...
    sportgruppe_to_edit = session.query(Sportgruppe).filter(Sportgruppe.Sportgruppen_ID == Sportgruppen_ID).first()
    
    if request.method == "POST":
        if edit_sportgruppe_form.validate_on_submit():
            Sportgruppen_ID = edit_sportgruppe_form.Sportgruppen_ID.data
            sportgruppe_to_edit = db.session.query(Sportgruppe).filter(Sportgruppe.Sportgruppen_ID == Sportgruppen_ID).first()
...
def deleteSportgruppe():
    ...
    if delete_item_form_obj.validate_on_submit():
        ...
        sportgruppe_to_delete = db.session.query(Sportgruppe).filter(Sportgruppe.Sportgruppen_ID == sportgruppe_code_to_delete)

...
def sportart_edit():
    ...
    sportart_to_edit = session.query(Sportart).filter(
        Sportart.Sportart_ID == Sportart_ID).first()
    if request.method == "POST":
        if edit_sportart_form.validate_on_submit():
            Sportart_ID = edit_sportart_form.Sportart_ID.data
            sportart_to_edit = db.session.query(Sportart).filter(
                Sportart.Sportart_ID == Sportart_ID).first()
...
def deleteSportart():
    ...
    if delete_item_form_obj.validate_on_submit():
        ...
        sportart_to_delete = db.session.query(Sportart).filter(
            Sportart.Sportart_ID == sportart_code_to_delete)
